...
module.exports = function(plugin, log, config) {
    ...
    plugin.plugins['covistra-mongodb'].indexManager.registerIndex(dbName, 'users', { username: 1});
    plugin.plugins['covistra-mongodb'].indexManager.registerIndex(dbName, 'users', { email: 1});
    ...
    User.prototype.confirmEmail = function(options) {
        ...
        return this.update({status: STATUS_VALUES.PENDING_FIRST_LOGIN}).then(function(result) {
            if(options.notify && Mailer) {
                ...
                Mailer.sendRichEmailToUser(config.get('plugins:security:mailer:from'), _this, {
    ...
    function _create(data) {
        ...
        return uniqueCheck(dbName, 'users', {username: data.username, email: data.email}).then(function() {
