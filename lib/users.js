...
module.exports = function(plugin, log, config) {
    ...
    if(Mailer) {
        ...
        TemplateEngine.addTemplatePath(path.resolve(__dirname, "..","templates"), { priority: 1});

        // Templates will be automatically resolved by the engine
        TemplateEngine.registerTemplate('verify-email', "verify-email.hbs");
        TemplateEngine.registerTemplate('welcome-email', "welcome-email.hbs");
    }
    else {
