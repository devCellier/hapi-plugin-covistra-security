...
module.exports = function(plugin, log, config) {
    ...
    var MongoDB = plugin.plugins['covistra-mongodb'];
    ...
    function _allocateToken(credentials, app, options) {
        log.debug("Allocating token for user %s and app %s for emitter %s", credentials.username, app.key, options.emitter || credentials._id);
        ...
        return _create({
            token: token,
            bearer: MongoDB.ObjectId(options.bearer || credentials._id.toString()),
            emitter: MongoDB.ObjectId(options.subject || credentials._id.toString()),
            app: app._id,
